...
const { getName, getBuffer, getJson, IdentifySong } = require('../Utilis/download');
const { emoji } = require('../Utilis/Misc');
const { audioCut } = require('../Utilis/fFmpeg');

Asena.addCommand({ pattern: 'whois ?(.*)', fromMe: true, desc: "Show Group or person info." }, (async (message, match) => {
...
Desc    : ${group.desc}` + '```';
		...
		return await message.sendMessage(buffer, { caption: msg }, MessageType.image);
		...
		} catch (error) { pp = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png' }
		let msg = '```' + `Name   : ${await getName(message.reply_message.jid, message.client)}
...
Status : ${status.status}` + '```';
		...
		let msg = '```' + `Name   : ${await getName(message.jid, message.client)}
Id     : ${message.reply_message.jid}
Status : ${status.status}`
		let buffer = await getBuffer(pp)
...
Asena.addCommand({ pattern: 'upload ?(.*)', fromMe: true, desc: "Download from link." }, (async (message, match) => {
	match = match == '' ? message.reply_message.text : match;
	if (match === '') return await message.sendMessage('```Give me a direct download link.```');
	...
	if (type == 'video') return await message.sendMessage(buffer, { filename: name, mimetype: mime }, MessageType.video);
	else if (type == 'image') return await message.sendMessage(buffer, { filename: name, mimetype: mime }, MessageType.image);
	else if (type == 'audio') return await message.sendMessage(buffer, { filename: name, mimetype: mime }, MessageType.audio);
	else return await message.sendMessage(buffer, { filename: name, mimetype: mime }, MessageType.document);
}));

Asena.addCommand({ pattern: 'scl ?(.*)', fromMe: true, desc: 'Download song SoundCloud.' }, (async (message, match) => {
	match = match == '' ? message.reply_message.text : match;
	if (match === '') return await message.sendMessage('```Give me a Link.```');
	if (!match.startsWith('https://')) return await message.sendMessage('*Give me a link.*');
	let sc = 'https://soundcloud.com' + match.split('.com')[1];
	let url = `https://api.zeks.xyz/api/soundcloud?apikey=bottus000000&url=${sc}`;
	const json = await getJson(url)
	if (json.status !== true) return await message.sendMessage('```Invalid Link.```');
	let title = json.result.title
	let { buffer, mime } = await getBuffer(json.result.download)
	await message.sendMessage(buffer, { filename: title, mimetype: mime, ptt: false }, MessageType.audio);
}));

Asena.addCommand({ pattern: 'emoji ?(.*)', fromMe: true, desc: "Convert emoji to sticker." }, (async (message, match) => {
	match = match == '' ? message.reply_message.text : match;
	if (match === '') return await message.sendMessage("*Give me a emoji.*");
	let buffer = await emoji(match)
	if (buffer !== false) return await message.sendMessage(buffer,
		{ quoted: message.quoted, mimetype: Mimetype.webp }, MessageType.sticker);
}));

Asena.addCommand({ pattern: 'ss ?(.*)', fromMe: true, desc: 'Take web  screenshot.' }, (async (message, match) => {
	match = match == '' ? message.reply_message.text : match;
	...
	await message.sendMessage(buffer, { quoted: message.quoted }, MessageType.image);
}));
...
Asena.addCommand({ pattern: 'attp ?(.*)', fromMe: true, desc: 'Text to sticker' }, (async (message, match) => {
	if (match === '') return await message.sendMessage('*Give me some words.*');
	let { buffer } = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(match)}`)
	if (buffer !== false) await message.sendMessage(buffer, { mimetype: Mimetype.webp }, MessageType.sticker);
}));
